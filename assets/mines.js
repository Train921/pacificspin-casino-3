...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-095cd2dc";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-095cd2dc">'+'<span class="tile-face-095cd2dc tile-front-095cd2dc"></span>'+'<span class="tile-face-095cd2dc tile-back-095cd2dc"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-095cd2dc")){return;}
var back=tile.querySelector(".tile-back-095cd2dc");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-095cd2dc");if(isMine){tile.classList.add("boom-095cd2dc");}
if(dim){tile.classList.add("dimmed-095cd2dc");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-095cd2dc")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-095cd2dc");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-095cd2dc")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-095cd2dc");void panelEl.offsetWidth;panelEl.classList.add("jolt-095cd2dc");}
endRound();}
